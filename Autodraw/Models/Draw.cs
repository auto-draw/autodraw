...
namespace Autodraw
{
    ...
    public class InputAction
    {
        ...
        public void PerformAction()
        {
            switch (Action)
            {
                ...
                case ActionType.KeyDown:
                    if (Enum.TryParse<KeyCode>(Data, true, out var keyCode))
                    {
                        Input.SendKeyDown(keyCode);
                    }
    ...
    public static class Drawing
    {
        ...
        public static async Task<bool> DrawStack(List<SKBitmap> stack, List<InputAction> actions, Vector2 position)
        {
            ...
            {
                if (e.Data.KeyCode == (KeyCode)Config.Keybind_StopDrawing) { StackHalted = true; }
            }
        ...
        public static async Task<bool> Draw(SKBitmap bitmap,Vector2 position)
        {
            ...
            {
                if (e.Data.KeyCode == (KeyCode)Config.Keybind_SkipRescan)
                {
                    if (NoRescan) return;
                    SkipRescan = true;
                }
            ...
            static void KeybindRelease(object? sender, KeyboardHookEventArgs e)
            {
                if (e.Data.KeyCode == (KeyCode)Config.Keybind_StopDrawing) Halt();
                if (e.Data.KeyCode == (KeyCode)Config.Keybind_SkipRescan)
                {
                    if (NoRescan) return;
                    SkipRescan = false;
                ...
                if (e.Data.KeyCode == (KeyCode)Config.Keybind_PauseDrawing) IsPaused = !IsPaused;
            }
            ...
            foreach (List<Vector2> Action in Actions)
            {
                ...
                Logs.Log($"Time per Action: {timeComp.TotalMilliseconds/Action.Count}");
                Logs.Log($"Action Count: {Action.Count}");
